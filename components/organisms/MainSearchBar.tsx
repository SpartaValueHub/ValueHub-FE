...
export function MainSearchBar({ className }: MainSearchBarProps) {
  return (
    ...
    >
      <button
        type="button"
        className="flex shrink-0 items-center gap-1 px-2 py-1 font-sans text-sm text-vh-gray-100"
        aria-label="카테고리 선택"
      >
        {MAIN_CATEGORY_PLACEHOLDER}
        <ChevronDown className="size-4" aria-hidden />
      </button>
      <span aria-hidden className="mx-3 h-5 w-px shrink-0 bg-vh-gray-100/40" />
      <input
        type="search"
        name="q"
        placeholder={MAIN_SEARCH_PLACEHOLDER}
        className="min-w-0 flex-1 bg-transparent font-sans text-sm text-vh-gray-100 outline-none placeholder:text-vh-gray-700"
