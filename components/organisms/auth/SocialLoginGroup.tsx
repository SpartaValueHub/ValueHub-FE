import { Button } from "@/components/atoms/button";
import { cn } from "@/lib/utils";

function GoogleIcon() {
  return (
    <svg aria-hidden viewBox="0 0 24 24" className="size-4">
      <path
        fill="currentColor"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
      />
      <path
        fill="currentColor"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="currentColor"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
      />
      <path
        fill="currentColor"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}

function KakaoIcon() {
  return (
    <svg aria-hidden viewBox="0 0 24 24" className="size-4">
      <path
        fill="currentColor"
        d="M12 3C6.48 3 2 6.58 2 11c0 2.84 1.87 5.33 4.68 6.78-.15.55-.97 3.54-1.01 3.7 0 0-.02.17.09.24.11.07.24.02.24.02.32-.04 3.68-2.42 4.27-2.83.59.08 1.2.12 1.73.12 5.52 0 10-3.58 10-8.03C22 6.58 17.52 3 12 3z"
      />
    </svg>
  );
}

interface SocialLoginGroupProps {
  className?: string;
}

/** 소셜 로그인 UI — OAuth BE 미연동 */
export function SocialLoginGroup({ className }: SocialLoginGroupProps) {
  return (
    <div className={cn("flex gap-5 md:gap-3", className)}>
      <Button
        type="button"
        variant="brand"
        className="h-auto flex-1 gap-2 rounded-sm border-[#d0d0d0] px-[30px] py-4 text-sm md:h-11 md:border-[#868686] md:px-8 md:py-0 md:text-base"
        disabled
        title="준비 중"
      >
        <span className="hidden md:inline-flex">
          <GoogleIcon />
        </span>
        구글 로그인
      </Button>
      <Button
        type="button"
        variant="brand"
        className="h-auto flex-1 gap-2 rounded-sm border-[#d0d0d0] px-[30px] py-4 text-sm md:h-11 md:border-[#868686] md:px-8 md:py-0 md:text-base"
        disabled
        title="준비 중"
      >
        <span className="hidden md:inline-flex">
          <KakaoIcon />
        </span>
        카카오 로그인
      </Button>
    </div>
  );
}
